...
**Deep_learning_EVO**: Contains the main codes (python and jupyter) for the machine learning part. Built using TensorFlow/Keras. The data used in the training/validation is include in the Data_EVO folder. Due to github's limitation, only .csv's are provided, but a code to transform to the .npz format is given. The jupyter notebook process imbalanced.ipynb reads and concat data and saves in .npz format for the EVO algorithm
